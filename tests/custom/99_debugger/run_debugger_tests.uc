...
function run_debugger(source_code, steps, timeout_sec) {
	...
	if (conn) {
		...
		// breakpoint at line 1). The quiet periods below are tuned short
		// (150/300ms) to keep the suite fast: the server emits each verb's
		// response as a tight burst with no deliberate pacing, so a burst
		// settles well inside that window even under valgrind. If a slow CI
		// host ever drops a late frame, nudge the second value up.
		for (let msg in drain_messages(conn, 150))
			push(messages, msg);
		...
		for (let msg in drain_messages(conn, 300))
			push(messages, msg);
...
level1();`,
		['BREAK {"spec":"level3"}', 'CONTINUE', 'BACKTRACE {}', 'CONTINUE'],
		{
			no_crash: true,
			check: (r) => {
				// Tail call optimization reuses the current call frame for the
				// callee, so the level1/level2 tail-call frames are collapsed
				// into the level3 frame rather than appearing as separate stack
				// entries. The backtrace therefore reports the current frame
				// (level3) plus a tco count of the frames folded into it.
				for (let m in r.messages) {
					if (m.verb != 'BACKTRACE') continue;
					for (let f in m.payload.frames) {
						if (f.function == 'level3' && f.tco >= 1)
							return null;
					}
				}
				return "expected level3 frame with a tco count (tail calls collapsed)";
			}
		}
	);
...
caller();`,
		['BREAK {"spec":"callee"}', 'CONTINUE', 'BACKTRACE {"full":true}', 'CONTINUE'],
		{
			no_crash: true,
			check: (r) => {
				// `caller() { return callee(); }` is a tail call, so the caller
				// frame is folded into the callee frame by TCO. The full
				// backtrace reports the current frame (callee) with a tco count
				// and, because full:true, its local variables.
				for (let m in r.messages) {
					if (m.verb != 'BACKTRACE') continue;
					for (let f in m.payload.frames) {
						if (f.function == 'callee' && f.tco >= 1 && f.variables)
							return null;
					}
				}
				return "expected callee frame with tco count and variables";
			}
		}
	);
...
print(result);`,
		[],
		// level1/level2 are tail calls, so TCO folds their frames into the
		// level3 frame; debug.traceback() therefore reports level3 (where the
		// traceback is taken) and main, but not the collapsed intermediate
		// frames.
		{ stdout_contains: ['done', 'level3', 'main'], no_crash: true }
	);
