...
/* Return the registered breakpoint object for `bk` if it is present in
 * vm->breakpoints, else NULL. A NULL result means `bk` was never registered
 * (e.g. the transient, stack-allocated attach-mode breakpoint in
 * uc_uloop_break_cb()) and must not be freed - its lifetime ends with the
 * caller's stack frame. */
static uc_breakpoint_t *
breakpoint_registered(uc_vm_t *vm, uc_breakpoint_t *bk)
{
	uc_breakpoints_t *bks = &vm->breakpoints;

	for (size_t i = 0; i < bks->count; i++)
		if (bks->entries[i] == bk)
			return bks->entries[i];
...
{
	/* Only free breakpoints that were actually registered in vm->breakpoints
	 * (i.e. heap-allocated via xalloc in patch_breakpoint()/get_breakpoint()).
	 * bk_enter_session() is also entered with transient, stack-allocated
	 * breakpoints (the attach-mode one in uc_uloop_break_cb()), which were
	 * never registered and must not be freed - their lifetime ends with the
	 * caller's stack frame.
	 *
	 * Free the pointer as loaded from the vector rather than the `bk`
	 * parameter: the static analyzer cannot otherwise prove that `bk` is
	 * heap-allocated and reports a false positive "free() of a stack
	 * variable" on the registered-breakpoint path. */
	uc_breakpoint_t *reg = breakpoint_registered(vm, bk);

	if (!reg)
		return false;

	unlink_breakpoint(vm, reg);
	free(reg);
